...
def get_filters():
    ...
    # User Input for City.
    city = ""
    while (city not in CITY_DATA_LIST):
        city = input("--> kindly choose which city do you want to find it\'s statistics Chicago, Washington or new york.\n\nInput : ").strip().lower()

    # User Input for Month.
    month = ""
    while (month not in MONTHS):
        month = input(
            "\n--> Select Month from (January, February , ..... June) or 'all' for no filter.\n\nInput : ").strip().lower()
        if (month == 'all'):
            break
    ...
    while (day not in DAYS_LIST):
        day = input(
            "\n--> Select Day from (Saturday, Sunday, ....., Friday) or 'all' for no filter.\n\nInput : ").strip().lower()
        if (day == 'all'):
            break
...
def load_data(city, month, day):
    ...
    df = pd.read_csv(CITY_DATA_LIST[city])
    ...
    df.rename(columns={'Start Time': 'start_time', 'End Time': 'end_time', 'Trip Duration': 'trip_duration',
                       'Start Station': 'start_station', 'End Station': 'end_station', 'User Type': 'user_type',
    ...
    # Day Only.
        ...
        return df[(df.start_time.dt.dayofweek == DAYS_LIST[day])]
    ...
    # Both Filtered.
        ...
        both_filtered = (df.start_time.dt.month == MONTHS[month]) & (df.start_time.dt.dayofweek == DAYS_LIST[day])
...
def time_stats(df):
    ...
    # Most Common Month.
    print(f"## Most Common Month : {df.start_time.dt.month_name().value_counts().idxmax()}.")

    # Most Common Day.
    print(f"## Most Common Day : {df.start_time.dt.day_name().value_counts().idxmax()}.")

    # Most Common Hour.
    print(f"## Most Common Hour : {df.start_time.dt.hour.value_counts().idxmax()}.")
